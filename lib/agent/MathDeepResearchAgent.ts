 ...
 * 数学学科深入分析智能体
 *
 * 实现基于 Deep Research 架构的数学学习分析系统
...
import {
  MATH_COMPREHENSIVE_ANALYSIS_PROMPT,
  MATH_ALGEBRA_ANALYSIS_PROMPT,
  MATH_STUDY_PLAN_PROMPT,
  MATH_AGENT_SYSTEM_PROMPT
} from './prompts/math-prompts'
import {
  searchMathKnowledgeNodes,
  getMathRelatedNodes,
  getMathPrerequisites
} from '@/lib/knowledge/math-knowledge-graph'
import type {
  AgentConfig,
  AgentResponse,
  ResearchPlan,
...
export class MathDeepResearchAgent {
  ...
  async analyze(params: {
    ...
    try {
      ...
      console.error('[MathDeepResearchAgent] Analysis error:', error)
      throw error
    }
  }
  ...
  private async createResearchPlan(params: any): Promise<ResearchPlan> {
    ...
    if (analysisType === 'comprehensive') {
      steps.push(
        ...
        },
        {
          ...
          tools: [],
          expectedOutput: '诊断结论',
          dependencies: ['analyze_errors', 'knowledge_analysis', 'ability_assessment', 'trend_analysis'],
          status: 'pending'
        },
        {
          id: 'generate_plan',
    ...
    return {
      objective: `分析学生数学学习状况（${analysisType}）`,
      steps,
      currentStep: 0,
      status: 'planning'
    }
  ...
  private async queryData(params: any): Promise<any> {
    ...
    // 筛选数学相关学科的试卷（包括代数、几何等）
    const mathExams = exams.filter(e => {
      const subject = (e.subject || '').toLowerCase()
      return ['math', '数学', 'mathematics', 'algebra', '代数', 'geometry', '几何', 'function', '函数'].includes(subject)
    })
    ...
    return {
      exams: mathExams,
      wrongQuestions,
      totalExams: mathExams.length,
      totalQuestions: mathExams.reduce((sum, e) => sum + (e.questions?.length || 0), 0),
  ...
  private async aiReasoning(
    ...
    // 选择合适的提示词
    const prompt = params.analysisType === 'algebra'
      ? MATH_ALGEBRA_ANALYSIS_PROMPT(analysisData)
      : MATH_COMPREHENSIVE_ANALYSIS_PROMPT(analysisData)
  ...
  private async generatePlan(
    ...
    const prompt = MATH_STUDY_PLAN_PROMPT({
  ...
  private async analyzeKnowledgeGaps(weakPoints: string[]): Promise<any> {
    ...
    for (const point of weakPoints) {
      const nodes = searchMathKnowledgeNodes([point])

      for (const node of nodes) {
        const prerequisites = getMathPrerequisites(node.id)
        const related = getMathRelatedNodes(node.id, 1)
  ...
  private calculateSkillScores(data: any): any {
    ...
    return {
      ...
      algebra: Math.round(accuracy * 10 * 0.95),
      geometry: Math.round(accuracy * 10 * 0.9),
      function: Math.round(accuracy * 10 * 0.85),
      statistics: Math.round(accuracy * 10 * 0.9)
    }
  }
 ...
 * 创建数学学科分析智能体实例
 */
export function createMathDeepResearchAgent(config?: AgentConfig): MathDeepResearchAgent {
  return new MathDeepResearchAgent(config)
}
